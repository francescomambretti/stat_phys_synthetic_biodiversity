...
### Counts the total HB and the MCO between all strand pairs as a function of time. The code creates a separate file for each pair, and also a separate plot.
...
simul_title="p4.6-p10-res" # might be changed
...
for a in range (0,tot_strands):
    for b in range(a,tot_strands):
        ...
        plt.title(simul_title+" ave over {} runs, strand {} vs {}".format(runs,a,b))

        plt.xlabel("{}$\\times $ MD steps ".format(freq))
        plt.ylabel("Overlap")
        ...
        plt.hist(all_mco_hb,bins=np.arange(0,51,1),rwidth=0.7,alpha=0.9,density=True,align='left',color='royalblue',edgecolor='black',label='MCO HB')
