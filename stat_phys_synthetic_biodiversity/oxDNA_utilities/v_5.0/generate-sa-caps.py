...
try:
    ...
    print >> sys.stderr, "Usage: %s <%s> <%s> <%s>" % (sys.argv[0], "box size", "file with sequences", "caps length [set 0 if you do not want them]")
    sys.exit(1)
...
class StrandGenerator (object):
    def generate(self, bp, sequence=None, start_pos=np.array([0, 0, 0]), dir=np.array([0, 0, 1]), perp=False, key="DOUBLE", skip=0, rot=0.):
...
def read_strands(filename):
    ...
    for line in lines:
        ...
        if len(line) == 0:
            continue
        if line[:6] == 'DOUBLE':
            line = line.split()[1].upper() #convert to uppercase
