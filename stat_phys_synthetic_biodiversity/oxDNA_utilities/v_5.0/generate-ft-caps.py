...
Create strands in user-defined positions, read from the start_pos.txt files

Edited by Francesco Mambretti: added the possibility to generate strands with caps on their ends (neither single nor double)
...
try:
    ...
    infile = sys.argv[2]
    caps_len = int(sys.argv[3])
except:
    print >> sys.stderr, "Usage: %s <%s> <%s>" % (sys.argv[0], "box size", "file with sequences", "caps length")
    sys.exit(1)
...
class StrandGenerator (object):
    def generate(self, bp, sequence=None, start_pos=np.array([0, 0, 0]), dir=np.array([0, 1, 0]), perp=False, key="DOUBLE", skip=0, rot=0.):
        ...
        if key=="DOUBLE":
            ...
            #translate up to the second cap
            rb+=skip*a3*BASE_BASE
                                                                                   
            for i in range(bp-caps_len,bp):
                rcdm = rb - CM_CENTER_DS * a1
                mynewpositions.append (rcdm)
                mynewa1s.append (a1)
                mynewa3s.append (a3)
...
def read_strands(filename):
    ...
    - Lines beginning in DOUBLE produce double stranded DNA
    - Lines beginning in CAPS produce single stranded DNA + caps at the ends
    ...
    # se e' un caps, ci deve essere CAPS - la taglia dei caps e' un nuovo argomento da riga di comando
    ...
    for line in lines:
        ...
        if len(line) == 0:
            continue
        if line[:6] == 'DOUBLE':
            line = line.split()[1]  #divide pieces separated by whitespaces 
            length = len(line)
            ...
            line = line.split()[1]
            length = len(line)
            print >> sys.stderr, "## Found single strand of %i bases with two %i-long caps" % (length, caps_len)
            nnucl += length + 2*caps_len
            nstrands += 3 #1 strand + 2 caps
        else:
            line = line.split()[0]
    ...
    for line in lines:
        ...
        if line[:6] == 'DOUBLE':
            ...
            print >> out, myns, line[0], -1, mynn + 1  #first one
            mynn += 1
            for i in xrange (1, len(line) - 1):
                print >> out, myns, line[i], mynn - 1, mynn + 1
                mynn += 1
            print >> out, myns, line[-1], mynn - 1, -1 #last one
            mynn += 1
            myns += 1
            ...
            # get the compl sequence in numbers
            caps_seq = [3 - base_to_number[x] for x in line]
            line_caps = [number_to_base[x] for x in caps_seq[::-1]] #reverse sequence from last to first
            
            print >> out, myns, line_caps[0], -1, mynn + 1 #first nucleotide of the first cap
            mynn += 1
            for i in xrange (1, caps_len - 1):
                print >> out, myns, line_caps[i], mynn - 1, mynn + 1
                mynn += 1
            print >> out, myns, line_caps[caps_len-1], mynn - 1, -1 #last nucleotide of the first cap
            mynn += 1
            myns += 1 #first cap
            
            skip=len(line_caps)-caps_len*2
            print >> out, myns, line_caps[skip+caps_len], -1, mynn + 1 #first nucleotide of the second cap
            mynn += 1
            for i in xrange (skip+caps_len+1, len(line_caps) - 1):
                print >> out, myns, line_caps[i], mynn - 1, mynn + 1
                mynn += 1
            print >> out, myns, line_caps[-1], mynn-1, -1 #last nucleotide of the second cap
            mynn += 1
            myns += 1 #second cap
    ...
    #counting line start_pos.txt
    count = 0
    #yesno = int(input("do you want to read start positions from file? 0/1 "))
    yesno = int(1)
    ...
    for line in lines:
        ...
        if line[:6] == 'DOUBLE':
            ...
            if (yesno == 0): #generate randomly placed strands
                cdm = np.random.random_sample(3) * box
                axis = np.random.random_sample(3)
                axis /= np.sqrt(np.dot(axis, axis))
                newpositions, newa1s, newa3s = double.generate(len(line), sequence=seq, dir=axis, start_pos=cdm, skip=0, key="DOUBLE")
            ...
            else: #read coordinates from start_pos.txt
                cdm = np.loadtxt("start_pos.txt",usecols=(0,1,2),dtype=float,skiprows=count,max_rows=1)
                axis = np.loadtxt("start_pos.txt",usecols=(3,4,5),dtype=float,skiprows=count,max_rows=1)
                count = count+1
                axis /= np.sqrt(np.dot(axis, axis))
                newpositions, newa1s, newa3s = double.generate(len(line), sequence=seq, dir=axis, start_pos=cdm, skip=0, key="DOUBLE")
                #control over cdm... if there are problems it will say that, I will do it random
                while not add_strands(newpositions, newa1s, newa3s):
                    print("doing random ")
                    cdm = np.random.random_sample(3) * box
                    axis = np.random.random_sample(3)
        ...
        #STRANDS WITH CAPS
            ...
            seq = [base_to_number[x] for x in line]
            length = len(line)
            print >> sys.stderr, "## Adding strand of %i bases with %i-long caps" % (length,caps_len)
                        
            if (yesno == 0): #generate randomly placed strands
                cdm = np.random.random_sample(3) * box
                axis = np.random.random_sample(3)
                axis /= np.sqrt(np.dot(axis, axis))
                newpositions, newa1s, newa3s = double.generate(len(line), sequence=seq, dir=axis, start_pos=cdm, skip=len(line)-caps_len*2, key="CAPS")
                while not add_strands(newpositions, newa1s, newa3s):
                    cdm = np.random.random_sample(3) * box
                    axis = np.random.random_sample(3)
                    axis /= np.sqrt(np.dot(axis, axis))
        ...
        #SINGLE STRANDS
            ...
            if (yesno == 0):
                ...
                axis = np.random.random_sample(3)
                axis /= np.sqrt(np.dot(axis, axis))
                newpositions, newa1s, newa3s = double.generate(len(line), sequence=seq, dir=axis, start_pos=cdm, skip=0, key="")
                while not add_strands(newpositions, newa1s, newa3s):
                    cdm = np.random.random_sample(3) * box
                    axis = np.random.random_sample(3)
                    axis /= np.sqrt(np.dot(axis, axis))
