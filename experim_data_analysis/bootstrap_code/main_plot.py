...
for fastq_file in fastq_file_list_plot:   #loop through cycles
    ...
    if(key3=="all"):
        ShEn_RSA_file=results_folder+"/Shannon_Entropy_RSA_all.txt" #do this only for "all" keyword
        if(t==0):
            os.system("rm "+ShEn_RSA_file)
        module_plots.abund_histo_ShEn_RSA(results_folder,fastq_file,-1,create_png_abund,ShEn_RSA_file)
