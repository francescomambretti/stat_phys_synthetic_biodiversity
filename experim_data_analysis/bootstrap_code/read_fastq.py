...
# 8) track the fraction of alien sequences
#
...
if (key2=="R1" or key2=="R2"):
    ...
    len_fq=len(fastq_file_list)
    tot_valid=np.zeros(len_fq)
    aliens=np.zeros(len_fq)
    unique_time_series=np.zeros(len_fq)
    
    for fastq_file in fastq_file_list:
        tot_valid[t],aliens[t]=func.process_FASTQ(fastq_file,mother_folder=mother_folder,results_folder=results_folder,stop=subset_steps,use_stop=use_stop,t=t)
    ...
    np.savetxt(results_folder+"/tot_valid.txt",np.column_stack((unique_time_series/tot_valid,tot_valid,aliens)),header="Unique sequences "+'\t'+" tot valid"+'\t'+" aliens")
    ...
    aliens=np.zeros(len(fastq_file_list_R1))
    for fastq_file_R1,fastq_file_R2 in zip(fastq_file_list_R1,fastq_file_list_R2):
        tot_valid[t],aliens[t]=func.process_FASTQ(fastq_file_R1,fastq_file_R2,mother_folder=mother_folder,results_folder=results_folder,stop=subset_steps,use_stop=use_stop,t=t)
    ...
    #print percentages of unique strands as a function of time
    print(unique_time_series/tot_valid)
    np.savetxt(results_folder+"/tot_valid.txt",np.column_stack((unique_time_series/tot_valid,tot_valid,aliens)),header="Unique sequences "+'\t'+" tot valid"+'\t'+" aliens", fmt="%.3f %d %d")
...
#do the plots
os.system("python main_plot.py") #os.system("python main_plot.py")
